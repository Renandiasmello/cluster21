...
include_once '..' . DS . 'classes' . DS . 'EntradaNumerica.php';

$obj_numerico = new EntradaNumerica();
$dados = $obj_numerico->retornarValor();

$retorno_save = false;
if(isset($_POST['submit'])){
	$retorno_save = $obj_numerico->adicionar($_POST['descricao']);
	$dados = $obj_numerico->retornarValor();
}

$retorno_delete = false;
if(isset($_GET['id'])){
	$retorno_delete = $obj_numerico->deletarValor($_GET['id']);
	$dados = $obj_numerico->retornarValor();
}
	...
	<div class="container">
		<div class="form-group">
			<form method="post" name="form">
				<fieldset>
			    <legend>Cadastro de Números</legend>
			    	<div class="control-group">
                        <label class="control-label">Descrição:</label>
                        <div class="controls">
                        	<!-- Não coloquei type number para verem a validação em funcionamento -->
                            <input type="text" class="form-control input1" name="descricao" id="descricao" aria-describedby="descricao" 
				    			placeholder="Preencha o valor numérico" value="" maxlength="100" autocomplete="off" required>
                            <span class="help-inline"></span>
                        </div>
                    </div>	
			    </fieldset><br>
			    <?php if(isset($_POST['submit']) && empty($_POST['descricao'])): ?>
		...
		<fieldset>
		<legend>Lista de valores numéricos</legend>
